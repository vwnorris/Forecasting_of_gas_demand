...
### Function to load and merge flow data with weather data ###
...
weather_data = pd.read_csv("merged_weather_data.csv", parse_dates=["time"])
merged_data = weather_data.merge(flow_data_hourly, left_on="time", right_on="Oslo Time", how="left")

# Drop the duplicate 'Oslo Time' column after merging
merged_data.drop(columns=["Oslo Time"], inplace=True)

# Save the final merged dataset
merged_data.to_csv("merged_weather_with_flow.csv", index=False)
print("Merged dataset saved as 'merged_weather_with_flow.csv'.")
